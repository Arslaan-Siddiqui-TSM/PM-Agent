...
class GraphRunner:
    ...
    Handles:
    - Graph initialization with persistent checkpointer
    ...
    _checkpointer: Any = None  # SqliteSaver type
    _checkpointer_cm: Any = None  # Context manager for cleanup
    ...
    def __init__(self):
        """Initialize the graph runner with persistent checkpointer."""
        if self._graph is None:
            self._initialize_graph()
    
    def _initialize_graph(self):
        """Initialize the compiled graph with SqliteSaver checkpointer."""
        logger = get_global_logger()
        
        # Use SqliteSaver for persistence (required for resume)
        checkpoint_path = Path("output/checkpoints/graph.db")
        ...
        try:
            ...
            checkpointer_cm = SqliteSaver.from_conn_string(str(checkpoint_path))
            ...
            if logger:
                logger.logger.info(f"Checkpointer initialized: {checkpoint_path}")
        ...
        # Get graph - pass our checkpointer to it
        self._graph = get_graph(dummy_state, checkpointer=self._checkpointer)
        
        if logger:
            logger.logger.info("GraphRunner initialized with compiled graph")
    ...
    def resume_execution(
        ...
        try:
            ...
            if logger:
                logger.logger.info(f"HITL Resume: Calling graph.invoke with Command(resume=...) for thread_id={thread_id}")
            ...
            if logger:
                ...
                logger.logger.debug(f"HITL Resume: Full result keys: {list(result.keys()) if isinstance(result, dict) else 'not a dict'}")
...
def get_graph_runner() -> GraphRunner:
    ...
    with a shared checkpointer for persistent state.
...
def cleanup_graph_runner():
    """
    Cleanup the GraphRunner singleton and close checkpointer connection.
    ...
    if _graph_runner is not None and _graph_runner._checkpointer_cm is not None:
        try:
            # Exit the context manager to close the connection
            _graph_runner._checkpointer_cm.__exit__(None, None, None)
            
            logger = get_global_logger()
            if logger:
                logger.logger.info("GraphRunner checkpointer connection closed")
        except Exception as e:
            logger = get_global_logger()
            if logger:
                logger.logger.warning(f"Error closing checkpointer: {e}")
        finally:
            _graph_runner = None
